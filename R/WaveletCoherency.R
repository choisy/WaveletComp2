...
function(x, y, dt = 1, dj = 1/20,
         ...
         window.type.t=1, window.type.s=1, window.size.t=5, window.size.s=1/4, log2scale = TRUE){

  #############################################################################
  ## compute wavelet transforms, retrieve results
  #############################################################################

  WT = WaveletTransform(x, dt = dt, dj = dj, lowerPeriod = lowerPeriod, upperPeriod = upperPeriod, log2scale)
  ...
  WT = WaveletTransform(y, dt = dt, dj = dj, lowerPeriod = lowerPeriod, upperPeriod = upperPeriod, log2scale)
