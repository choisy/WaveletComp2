...
function(WC, which.avg = "wp", exponent = 1,
  ...
  period <- WC$Period
  if (WC$log2scale) period <- log2(period)

  plot(W.avg, period, xlim = range(c(W.avg,minimum.level,maximum.level)),
  ...
  if ((show.siglvl == T) & (is.null(W.avg.pval) == F)) {
     ...
     P.dat = data.frame(Pvalue = W.avg.pval, Log.period = period, Average = W.avg)
