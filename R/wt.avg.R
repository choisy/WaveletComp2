...
function(WT, my.series = 1, exponent = 1,
  ...
  period <- WT$Period
  if (WT$log2scale) period <- log2(period)

  plot(Power.avg, period, xlim = range(c(Power.avg,minimum.level,maximum.level)),
  ...
  if ((show.siglvl == T) & (is.null(Power.avg.pval) == F)) {
     ...
     P.dat = data.frame(Pvalue = Power.avg.pval, Log.period = period, Average = Power.avg)
  ...
  if (label.period.axis == T) {
      ...
      if (period.axis.default) {
        if (WT$log2scale) {
          ...
          period.tick[period.tick<period[1]] = NA
       ...
       if (!period.axis.default) {
          period.tick = spec.period.axis$at
          if (WT$log2scale) period.tick <- log2(period.tick)
          period.tick[(period.tick<period[1])] = NA
